...
func TestMigrateLegacyLayout_CopiesSplitCategories(t *testing.T) {
	...
	assertMigrationFile(t, filepath.Join(home, ".config", "groundskeeper", "config.toml"), "theme = \"dark\"\n")
	assertMigrationFile(t, filepath.Join(home, ".local", "share", "groundskeeper", "profiles", "default", "state.db"), "db")
	assertMigrationFile(t, filepath.Join(home, ".cache", "groundskeeper", "update-cache.json"), "{}")
...
func TestMigrateLegacyLayout_ConflictRequiresForceAndLeavesFilesUntouched(t *testing.T) {
	...
	xdgConfig := filepath.Join(home, ".config", "groundskeeper", "config.toml")
...
func TestMigrateLegacyLayout_DryRunDoesNotCopy(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(home, ".config", "groundskeeper", "config.toml")); !os.IsNotExist(err) {
...
func TestMigrateLegacyLayout_ForcePreservesNewerXDGOnlyData(t *testing.T) {
	...
	xdgProfiles := filepath.Join(home, ".local", "share", "groundskeeper", "profiles")
...
func TestMigrateLegacyLayout_ForcePrefersNewerXDGOnPerFileConflict(t *testing.T) {
	...
	xdgFile := filepath.Join(home, ".local", "share", "groundskeeper", "profiles", "default", "state.db")
