...
func TestLogCgroupIsolationDecision_WiredIntoBootstrap(t *testing.T) {
	...
	t.Run("tui_startup_emits_line", func(t *testing.T) {
		...
		if err := os.MkdirAll(filepath.Join(xdgCacheHome, "groundskeeper"), 0o755); err != nil {
			t.Fatal(err)
		}
		binPath := filepath.Join(t.TempDir(), "groundskeeper-test")
		...
		logPath := filepath.Join(xdgCacheHome, "groundskeeper", "debug.log")
