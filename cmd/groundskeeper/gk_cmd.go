...
func handleSetup(args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Println()
		fmt.Println("Configure the full Groundskeeper stack: OMP, Espalier Core,")
		fmt.Println("the worker model, and the local gk.db state store.")
		fmt.Println()
		fmt.Println("Safe to re-run: existing installs, Espalier checkouts, and")
		fmt.Println("credential stores are discovered and reused when possible.")
		fmt.Println()
		fmt.Println("Options:")
		fmt.Println("  --non-interactive")
		fmt.Println("        Skip all prompts. Missing optional pieces are reported unless")
		fmt.Println("        --install-missing is also set.")
		...
		fmt.Println("  groundskeeper setup --non-interactive --install-missing")
		fmt.Println("  groundskeeper setup --non-interactive --install-missing --model ollama-cloud/glm-5.2 --verify-model")
	}
	if err := fs.Parse(args); err != nil {
		os.Exit(2)
	}
