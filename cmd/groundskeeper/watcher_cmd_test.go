...
func TestWatcherInstallSkill(t *testing.T) {
	...
	poolDir := filepath.Join(xdgConfigHome, "groundskeeper", "skills", "pool", "watcher-creator")
...
func TestWatcherInstallSkill_DirMode(t *testing.T) {
	...
	dirs := []string{
		filepath.Join(xdgConfigHome, "groundskeeper", "skills"),
		filepath.Join(xdgConfigHome, "groundskeeper", "skills", "pool"),
		filepath.Join(xdgConfigHome, "groundskeeper", "skills", "pool", "watcher-creator"),
	}
	for _, d := range dirs {
		info, err := os.Stat(d)
		if err != nil {
