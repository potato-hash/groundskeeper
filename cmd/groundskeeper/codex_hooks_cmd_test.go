...
func TestHandleCodexNotify_WritesStatus(t *testing.T) {
	...
	os.Args = []string{"groundskeeper", "codex-notify"}
...
func TestHandleCodexNotify_ArgPayload(t *testing.T) {
	...
	origArgs := os.Args
	defer func() { os.Args = origArgs }()
	os.Args = []string{"groundskeeper", "codex-notify", `{"event":"turn/completed","thread_id":"thr-1"}`}
...
func TestHandleCodexNotify_JSONRPCMethodPayload(t *testing.T) {
	...
	origArgs := os.Args
	defer func() { os.Args = origArgs }()
	os.Args = []string{"groundskeeper", "codex-notify", `{"method":"turn/completed","params":{"thread_id":"thr-42"}}`}
...
func TestHandleCodexNotify_EmptyTailEventKeepsJSONEmptyAndPersistsAnchor(t *testing.T) {
	...
	os.Args = []string{"groundskeeper", "codex-notify", `{"event":"turn/started","thread_id":"thr-sticky"}`}
	handleCodexNotify()

	// Tail event has no session_id/thread_id; should backfill from sticky store.
	os.Args = []string{"groundskeeper", "codex-notify", `{"event":"turn/completed"}`}
	handleCodexNotify()
