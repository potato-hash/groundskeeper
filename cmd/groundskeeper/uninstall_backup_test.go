...
func TestBackupUninstallDataLocations_IncludesXDGData(t *testing.T) {
	...
	xdgConfig := filepath.Join(homeDir, ".config", "groundskeeper")
	xdgData := filepath.Join(homeDir, ".local", "share", "groundskeeper")
	xdgCache := filepath.Join(homeDir, ".cache", "groundskeeper")
